...
export class QuotationCreateModuleComponent implements OnInit {
    ...
    ngOnInit() {
        this.translate
            .get(['Quote Number', 'Rejected Reason', 'Updated On', 'Additional Charges', 'Additional Charge Cost', 'Total Updated cost', 'Action'])
    ...
    expense_header() {
        this.charges = this.charges != null ? this.charges : 0.0;
    ...
    selected_expense(event: any) {
        ...
        this.charges = this.charges != 0 ? this.charges : 0.0;
        this.addChargeFlag = (this.charges_text && this.charges != -1) || (!this.charges_text && this.charges == 0) ? false : true;
    }
    ...
    check_valid() {
        ...
        const itemsValid = this.quote_items.some(
            (item: { servpack_id: string; servpack_cost: string }) => Number(item.servpack_id) === 0 || Number(item.servpack_cost) === 0
        );
