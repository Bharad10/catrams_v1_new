...
export class WorkCardEditModuleComponent implements OnInit {
        ...
        if (this.request_id) {
            ...
            this.usr_ser.fetch_service_details(this.request_id).subscribe((rdata: any) => {
                if (rdata.ret_data == 'success') {
                    ...
                    if (
                        rdata.services.every((d: any) => {
                            if (d.sitem_type === '0') {
                                // Check only the conditions for sitem_type === '0'
                                return (
                                    d.sitem_hold_flag === '0' &&
                                    (d.sitem_active_flag == '3' || (d.sitem_active_flag == '1' && (d.sitem_status_flag == '2' || d.sitem_status_flag == '6')))
                                );
                            } else if (d.sitem_type === '1') {
                                // Check all the conditions for sitem_type === '1'
                                return (
                                    d.sitem_status_flag == '3' ||
                                    d.sitem_status_flag == '5' ||
                                    (d.sitem_status_flag == '4' && d.requet_details && d.requet_details.sm_id == '16')
                                );
                            } else {
    ...
    addItem() {
        ...
        console.log('quotes>>>>>>>>>>>>>>>>>>>>', this.quote_items);

        const allValid = this.quote_items.every((item: any) => item.amount !== 0);
        if (allValid) {
            ...
            this.showMessage('Please fill all missing fields in service items', 'error');
        }
    }
    ...
    searchUsers() {
        return this.contactList.filter((contact: { cstm_name: string }) => {
            return contact.cstm_name.toLowerCase().includes(this.searchUser.toLowerCase());
        });
    }

    searchUserrsexp() {
        return this.exp_contactList.filter((contact: { cstm_name: string }) => {
            return contact.cstm_name.toLowerCase().includes(this.searchUser.toLowerCase());
        });
    }
    ...
    getUserId(data: any) {
        ...
        let room_id =
            this.base_version == 'test'
                ? 'RAMSTEST_SRC_' + data.cstm_id + '_' + this.request_details['serm_id']
                : 'RAMS_SRC_' + data.cstm_id + '_' + this.request_details['serm_id'];
    ...
    sendMessage(message: any, type: any) {
        ...
        let room_id =
            this.base_version == 'test'
                ? 'RAMSTEST_SRC_' + userinfo['cstm_id'] + '_' + this.request_details['serm_id']
                : 'RAMS_SRC_' + userinfo['cstm_id'] + '_' + this.request_details['serm_id'];
